...
const dict_max = "MainList"; // Dictionary in Max
var dict_js = {}; // Max dictionary to JS object
...
// Copies file from one place to another
function format_and_copy_files(from, to, base_folder_and_file, audio_file) {
    // If there are languages selected
    if (languages.length > 0) {
        // For each language in the array
        languages.forEach(language => {
            maxAPI.post("From1: " + from);
            // Final format of source path (with file).
            from2 = path.normalize(path.join(from, audio_file));
            // Final format of destination path (with file).
            to2 = path.normalize(path.join(to, language, audio_file));
            // If the source file exists
            maxAPI.post("From2: " + from2);
            maxAPI.post("To2: " + to2);
            if (fs.existsSync(from2)) {
                // Copy the files
                create_path_and_copy_files(from2, to2)
        ...
        // Destination format
        to3 = path.normalize(path.join(to, "../../", base_folder_and_file, audio_file));
        // Path format for non project files
        from3 = path.normalize(path.join(from, "../", audio_file));
        create_path_and_copy_files(from3, to3)
        maxAPI.post("Source: " + from3)
...
async function dictBase() {
    dict_js = await maxAPI.getDict(dict_max);
    // Get array of keys. Array of strings.
    folder_list = Object.keys(dict_js)
    // Get array of values. Array of arrays.
    ...
    for (let i = 0; i < file_list.length; i++) {
        for (let j = 0; j < file_list[i].length; j++) {
            ...
            maxAPI.post("Before: " + path_base);
            path_base = path.join(source_directory, path_base);
            maxAPI.post("After: " + path_base);
            format_and_copy_files(source_directory, destination_directory, path_base, path.join(folder_list[i], file_list[i][j]));
        }
        // maxAPI.post("~~~~~")
    }
}
